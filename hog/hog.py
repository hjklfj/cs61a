...
def perfect_square(num):
    ...
    assert num > 0, 'num must be bigger than zero'
    if num == 1:
        return True
    i=2
    while i < num:
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score0 = update(num_of_rolls0, score0, score1, dice)
        else:
            num_of_rolls1 = strategy1(score1, score0)
            score1 = update(num_of_rolls1, score1, score0, dice)
        who = 1 - who
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while i<goal:
        ...
        while j < goal:
            if i==0 and j==0:
                first_res = strategy(i, j)
            elif first_res != strategy(i, j):
                return False
            j+=1
        i+=1
    return True
...
def make_averaged(original_function, total_samples=1000):
    ...
    3.0
    """
    def roll_dice_averaged(*args):
        i, total = 0, 0
        while i < total_samples:
            total += original_function(*args)
            i += 1
        return total / total_samples
    return roll_dice_averaged; 
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    average_version = make_averaged(roll_dice,  total_samples)
    i, r = 1, 1
    avg_score = 0
    while i <= 10:
        avg_score_temp = average_version(i, dice)
        if avg_score_temp > avg_score:
            avg_score = avg_score_temp
            r = i
        i+=1
    return r
    # END PROBLEM 9
